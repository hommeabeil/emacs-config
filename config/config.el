...
(defun erlfmt-indent-buffer ()
  ...
  (let* ((file (buffer-file-name))
    ...
    (unwind-protect
        (call-process-region (point-min) (point-max)
                             "/home/jbouchard/.local/bin/erlfmt" t t nil "--print-width" "120" "-")
      (dotimes (index (length orig-windows))
        (set-window-start (nth index orig-windows)
                          (nth index orig-window-starts)))
      (goto-char orig-point))))
...
(defun add-my-keys
  ...
  (spacemacs/set-leader-keys-for-major-mode 'erlang-mode "M-q" 'erlfmt-indent-buffer)
